...
DMG_OUT="$BUILD_DIR/Sige-2.0.dmg"
...
# 5. Clean metadata and sign the app bundle
xattr -cr "$STAGING/Sige.app" 2>/dev/null || true
codesign --force --deep --sign - "$STAGING/Sige.app"
codesign --verify --deep --strict --verbose=2 "$STAGING/Sige.app"

# 6. Create Applications symlink
...
# 8. Copy to product page
cp "$DMG_OUT" "$BUILD_DIR/product-page/Sige-2.0.dmg"
